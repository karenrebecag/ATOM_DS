...
/**
 * WhatsAppButtonComponent (Integration — Slater)
 *
 ...
 *
 * @example
 * <atom-whatsapp-button phone="573142616335" cta="hablar_asesor" />
 * <atom-whatsapp-button phone="573142616335" cta="agendar_demo" lang="en" label="BOOK A DEMO" />
 */
...
// ── WhatsApp SVG path (inline, no Font Awesome dependency) ─
...
export class WhatsAppButtonComponent {
  ...
  cta      = input<string>('hablar_asesor');
  /** Button label text. */
  label    = input<string>('HABLAR CON ASESOR');
  /** Custom WA message — overrides Slater's localized default. */
  message  = input<string | undefined>(undefined);
  /** Language override for Slater message resolution. */
  lang     = input<'es' | 'en' | 'pt' | undefined>(undefined);
  /** Analytics: page section where the button lives. */
  section  = input<string | undefined>(undefined);
  /** Analytics: industry classification. */
  industry = input<string | undefined>(undefined);
  /** Additional CSS classes. */
  class    = input<string | undefined>(undefined);

  readonly waSvgPath = WA_SVG_PATH;

  readonly href = computed(() => {
    const msg = this.message();
    return `https://wa.me/${this.phone()}${msg ? `?text=${encodeURIComponent(msg)}` : ''}`;
  });
  ...
  constructor() {
    if (isPlatformBrowser(this.platformId)) {
      afterNextRender(() => {
        this.loadSlaterAssets();
      });
    }
  }

  private loadSlaterAssets() {
